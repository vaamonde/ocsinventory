...
mysqldump --add-drop-table --complete-insert --extended-insert --quote-names --host=$HOST --user=$USER --password=$PWD --databases $DB > $PATH_1/glpi-$DATA.sql
echo -e "Backup feito com sucesso!!!, continuando o backup..." &>> $LOG

echo -e "Compactando o Backup da Base de Dados: $DB" &>> $LOG
tar -zcvf $PATH_1/glpi-$DATA.tar.gz $PATH_1/glpi-$DATA.sql &>> $LOG
